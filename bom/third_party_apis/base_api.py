...
class BaseApi:
    def __init__(
        self, api_settings_key, root_url, api_key_query=None, cache_timeout=86400
    ):
        self.api_key = None
        ...
        try:
            ...
            raise ValueError(
                "No API key for {} found on server. Contact administrator for help.".format(
                    api_settings_key
                )
            )
    ...
    def request(self, suburl, data=None):
        ...
        if self.api_key_query is None or self.api_key is None:
            raise ValueError(
                "No API key, or api key query found on server. Contact administrator for help."
            )
        ...
        if r.status_code != 200:
            raise BaseApiError(
                f"HTTP Response != 200. Returned: {r.status_code} {r.reason}"
            )
