...
class Mouser:
    ...
    def search_and_match(self, manufacturer_part, quantity=1, currency=None):
        ...
        if manufacturer:
            ...
            #  to reduce results
            mfg_id = (
                manufacturer_list[manufacturer.name]
                if manufacturer.name in manufacturer_list
                else None
            )
            if mfg_id:
                results = self.api.search_part_and_manufacturer(
                    part_number=manufacturer_part_number, manufacturer_id=mfg_id
                )
            else:
        ...
        for part in results["Parts"]:
            ...
            try:
                quantity_available = [
                    int(s) for s in part["Availability"].split() if s.isdigit()
                ][0]
                mouser_part = {
