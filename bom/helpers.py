...
def create_user_and_organization(
    ...
    organization = create_a_fake_organization(
        user, free, number_scheme, number_variation_len
    )
...
def create_some_fake_part_classes(organization):
    pc1, c = PartClass.objects.get_or_create(
        code=500, name="Wendy", comment="Mechanical Switches", organization=organization
    )
    pc2, c = PartClass.objects.get_or_create(
        code=200, name="Archibald", comment="", organization=organization
    )
    pc3, c = PartClass.objects.get_or_create(
        code="50A", name="Ghost", comment="Like Kasper", organization=organization
    )
    return pc1, pc2, pc3


def create_a_fake_subpart(part_revision, reference="U1", count=4):
    sp1 = Subpart(part_revision=part_revision, reference=reference, count=count)
    sp1.save()
...
def create_some_fake_manufacturers(organization):
    m1, c = Manufacturer.objects.get_or_create(
        name="STMicroelectronics", organization=organization
    )
    m2, c = Manufacturer.objects.get_or_create(
        name="Nordic Semiconductor", organization=organization
    )
...
def create_a_fake_seller_part(
    seller, manufacturer_part, moq, mpq, unit_cost, lead_time_days, nre_cost=None
):
    sp1 = SellerPart(
        seller=seller,
...
def create_some_fake_intelligent_parts(organization):
    pt1 = Part(
        number_item=("3" * organization.number_item_len), organization=organization
    )
    pt1.save()

    pt2 = Part(
        number_item="4" * organization.number_item_len, organization=organization
    )
    pt2.save()

    pt3 = Part(
        number_item="A" * organization.number_item_len, organization=organization
    )
    pt3.save()

    # pt4 is a part with no PartRevision
    pt4 = Part(
        number_item="B" * organization.number_item_len, organization=organization
    )
    pt4.save(no_part_revision=True)

    pt5 = Part(
        number_item="5" * organization.number_item_len, organization=organization
    )
    pt5.save()
...
def create_some_fake_parts(organization):
    ...
    mp1 = ManufacturerPart(
        part=pt1, manufacturer=m1, manufacturer_part_number="STM32F401CEU6"
    )
    mp1.save()
    ...
    mp2 = ManufacturerPart(
        part=pt2, manufacturer=m2, manufacturer_part_number="GRM1555C1H100JA01D"
    )
    mp2.save()
    mp22 = ManufacturerPart(
        part=pt2,
        manufacturer=None,
        manufacturer_part_number="ThisPartHasNoManufacturer",
    )
    mp22.save()
    ...
    mp3 = ManufacturerPart(
        part=pt3, manufacturer=m3, manufacturer_part_number="NRF51822"
    )
    mp3.save()
    ...
    create_a_fake_seller_part(
        s1, mp1, moq=1, mpq=1, unit_cost=1.2, lead_time_days=20, nre_cost=500
    )
    create_a_fake_seller_part(
        s2, mp1, moq=1000, mpq=5000, unit_cost=0.1005, lead_time_days=7, nre_cost=0
    )
    create_a_fake_seller_part(
        s2, mp2, moq=200, mpq=200, unit_cost=0.5, lead_time_days=47, nre_cost=1
    )
    create_a_fake_seller_part(
        s2, mp2, moq=2000, mpq=200, unit_cost=0.4, lead_time_days=47, nre_cost=10
    )
    create_a_fake_seller_part(
        s1, mp2, moq=2000, mpq=200, unit_cost=0.4, lead_time_days=47, nre_cost=10
    )
    create_a_fake_seller_part(
        s1, mp2, moq=3000, mpq=200, unit_cost=0.3, lead_time_days=47, nre_cost=10
    )
...
def create_all_part_classes():
    PartClass.objects.get_or_create(
        code=100,
        name="Assembly, Top Level SKU's, Finished Goods",
        comment="Ready to ship product; Includes packout and literature",
    )
    PartClass.objects.get_or_create(
        code=101,
        name="Assembly, Final, Elec/Mech",
        comment="Fully built product without Literature or packout materials",
    )
    PartClass.objects.get_or_create(
        code=102, name="Assembly, Sub-Assy, Elec/Mech", comment=""
    )
    ...
    PartClass.objects.get_or_create(
        code=105,
        name="Kit, Spare/Upgrade",
        comment="Includes Spares, Documentation Kits, Upgrade Kits",
    )
    PartClass.objects.get_or_create(
        code=106, name="Assembly, Printed Circuit Board (PBCA)", comment=""
    )
    PartClass.objects.get_or_create(code=107, name="PCB, Fab", comment="")
    PartClass.objects.get_or_create(
        code=108, name="Gerber File/X-Y data/CAD data", comment=""
    )
    PartClass.objects.get_or_create(code=109, name="Schematic/Test Diagram", comment="")
    PartClass.objects.get_or_create(
        code=110, name="Mechanical Reference, PCB", comment=""
    )
    PartClass.objects.get_or_create(
        code=111, name="Mechanical Reference, PCBA", comment=""
    )
    PartClass.objects.get_or_create(
        code=112, name="Programmed Device, Firmware/Software", comment=""
    )
    PartClass.objects.get_or_create(
        code=113, name="Program File, Firmware, Software", comment=""
    )
    PartClass.objects.get_or_create(
        code=114, name="Build-To-Print, Sheet Metal", comment=""
    )
    PartClass.objects.get_or_create(
        code=115, name="Build-To-Print, Cast Metal", comment=""
    )
    PartClass.objects.get_or_create(
        code=116,
        name="Build-To-Print, Extruded",
        comment="Applies for both plastic and metal",
    )
    PartClass.objects.get_or_create(
        code=117,
        name="Build-To-Print, Machined",
        comment="Applies for both plastic and metal",
    )
    PartClass.objects.get_or_create(
        code=118,
        name="Build-To-Print, Molded",
        comment="Applies for both Injection and Compression molded",
    )
    PartClass.objects.get_or_create(
        code=119,
        name="Build-To-Print, Formed",
        comment="Applies for plastic only; Formed sheet metal goes under sheet metal",
    )
    PartClass.objects.get_or_create(
        code=120,
        name="Build-To-Print, Die-Cut",
        comment="Applied to foams and PSAs (Pressure Sensitive Adhesives)",
    )
    PartClass.objects.get_or_create(
        code=121,
        name="Build-To-Print, Mechanical",
        comment="Applies to all Build-To-Print Mech Parts that do not fall into one of the other specific groups.",
    )
    PartClass.objects.get_or_create(
        code=122,
        name="Build-To-Print, Printed Material",
        comment="Includes Custom Labels, Product Literature, etc.",
    )
    PartClass.objects.get_or_create(
        code=123, name="Build-To-Print, Packaging", comment=""
    )
    PartClass.objects.get_or_create(
        code=124,
        name="Drawing",
    ...
    PartClass.objects.get_or_create(
        code=126, name="Release Notes/Protocols", comment=""
    )
    PartClass.objects.get_or_create(
        code=127, name="Custom Electronic Components", comment="Example: Custom Sensors"
    )
    PartClass.objects.get_or_create(
        code=200, name="Adhesive", comment="Includes Loctite, Epoxy, Tape, etc."
    )
    ...
    PartClass.objects.get_or_create(
        code=203, name="Connector, Cable/Harness", comment=""
    )
    PartClass.objects.get_or_create(
        code=204, name="Connector, PC Mountable", comment=""
    )
    PartClass.objects.get_or_create(code=205, name="Connector, Misc", comment="")
    PartClass.objects.get_or_create(
        code=206, name="Cable Hardware", comment="Includes Clamps, Ties, etc."
    )
    ...
    PartClass.objects.get_or_create(
        code=227, name="Terminal", comment="Includes Ring, Spade, Butt, etc."
    )
    ...
    PartClass.objects.get_or_create(
        code=233, name="Enclosures", comment="Off-The-Shelf Enclosures"
    )
