...
class MouserPartMatchBOM(BomJsonResponse):
    def get(self, request, part_revision_id):
        part_revision = get_object_or_404(
            PartRevision, pk=part_revision_id
        )  # get all of the pricing for manufacturer parts, marked with mouser in this part
        user = request.user
        ...
        for bom_id, mp in manufacturer_parts.items():
            ...
            try:
                part_seller_info = mouser.search_and_match(
                    mp, quantity=bom_part_quantity, currency=organization.currency
                )
