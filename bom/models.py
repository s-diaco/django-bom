...
class Organization(models.Model):
    ...
    number_class_code_len = models.PositiveIntegerField(
        default=NUMBER_CLASS_CODE_LEN_DEFAULT,
        validators=[
            MinValueValidator(NUMBER_CLASS_CODE_LEN_MIN),
            MaxValueValidator(NUMBER_CLASS_CODE_LEN_MAX),
        ],
    )
    number_item_len = models.PositiveIntegerField(
        default=NUMBER_ITEM_LEN_DEFAULT,
        validators=[
            MinValueValidator(NUMBER_ITEM_LEN_MIN),
            MaxValueValidator(NUMBER_ITEM_LEN_MAX),
        ],
    )
    number_variation_len = models.PositiveIntegerField(
        default=NUMBER_VARIATION_LEN_DEFAULT,
        validators=[
            MinValueValidator(NUMBER_VARIATION_LEN_MIN),
...
class UserMeta(models.Model):
    ...
    def get_or_create_organization(self):
        if self.organization is None:
            ...
            organization, created = Organization.objects.get_or_create(
                owner=self.user, defaults={"name": org_name, "subscription": "F"}
            )
    ...
    def _user_meta(self, organization=None):
        return UserMeta.objects.get_or_create(
            user=self, defaults={"organization": organization}
        )[0]
...
class PartClass(models.Model):
    ...
    code = models.CharField(
        max_length=NUMBER_CLASS_CODE_LEN_MAX, validators=[alphanumeric]
    )
    name = models.CharField(max_length=255, default=None)
...
class Part(models.Model):
    ...
    number_class = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="number_class",
        on_delete=models.CASCADE,
        db_index=True,
    )
    number_item = models.CharField(
        max_length=NUMBER_ITEM_LEN_MAX, default=None, blank=True
    )
    number_variation = models.CharField(
        max_length=NUMBER_VARIATION_LEN_MAX,
        default=None,
        blank=True,
        null=True,
        validators=[alphanumeric],
    )
    primary_manufacturer_part = models.ForeignKey(
        "ManufacturerPart",
        default=None,
        null=True,
        blank=True,
    ...
    def verify_format_number_class(number_class, organization):
        if len(number_class) != organization.number_class_code_len:
            raise AttributeError(
                f"Expect {organization.number_class_code_len} digits for number class"
            )
        elif number_class is not None:
            for c in number_class:
                if not (c.isdigit() or c.isalpha()):
                    raise AttributeError(
                        f"{c} is not a proper character for a number class"
                    )
        return number_class
    ...
    def verify_format_number_variation(number_variation, organization):
        if len(number_variation) != organization.number_variation_len:
            raise AttributeError(
                f"Expect {organization.number_variation_len} characters for number variation"
            )
        elif number_variation is not None:
            for c in number_variation:
                if not c.isalnum():
                    raise AttributeError(
                        f"{c} is not a proper character for a number variation. Must be alphanumeric."
                    )
        ...
        if organization.number_scheme == NUMBER_SCHEME_SEMI_INTELLIGENT:
            try:
                (
                    ...
                    number_item,
                    number_variation,
                ) = Part.parse_partial_part_number(part_number, organization)
            except IndexError:
                raise AttributeError(
                    "Invalid part number. Does not match organization preferences."
                )
            ...
            if (
                number_variation is None
                and organization.number_class_code_len != 0
                and organization.number_variation_len > 0
            ):
    ...
    def parse_partial_part_number(part_number, organization, validate=True):
        if organization.number_scheme == NUMBER_SCHEME_SEMI_INTELLIGENT:
            ...
            if validate:
                if len(elements) >= 2:
                    number_class = Part.verify_format_number_class(
                        elements[0], organization
                    )
                    number_item = Part.verify_format_number_item(
                        elements[1], organization
                    )
                if len(elements) >= 3:
                    number_variation = Part.verify_format_number_variation(
                        elements[2], organization
                    )
    ...
    def from_part_number(cls, part_number, organization):
        (number_class, number_item, number_variation) = Part.parse_part_number(
            part_number, organization
        )
        if organization.number_scheme == NUMBER_SCHEME_SEMI_INTELLIGENT:
        ...
        return Part.objects.get(number_item=number_item, organization=organization)
    ...
    def seller_parts(self, exclude_primary=False):
        ...
        q = (
            SellerPart.objects.filter(manufacturer_part__in=manufacturer_parts)
            .order_by("seller", "minimum_order_quantity")
            .select_related("manufacturer_part")
            .select_related("manufacturer_part__manufacturer")
            .select_related("seller")
        )
        if (
            exclude_primary
            and self.primary_manufacturer_part is not None
            and self.primary_manufacturer_part.optimal_seller()
        ):
    ...
    def where_used(self):
        ...
        used_in_assembly_ids = AssemblySubparts.objects.filter(
            subpart__in=used_in_subparts
        ).values_list("assembly", flat=True)
    ...
    def assign_part_number(self):
        if self.number_item is None or self.number_item == "":
            last_number_item = (
                Part.objects.filter(
                    number_class=self.number_class, organization=self.organization
                )
                .order_by("number_item")
                .last()
            )
        ...
        if (
            self.number_variation is None or self.number_variation == ""
        ) and self.organization.number_variation_len > 0:
            last_number_variation = (
                Part.objects.all()
                .filter(number_class=self.number_class, number_item=self.number_item)
                .order_by("number_variation")
                .last()
            )
    ...
    def __str__(self):
        return "%s" % (self.full_part_number())
...
class PartRevision(models.Model):
    ...
    displayable_synopsis = models.CharField(
        editable=False, default="", null=True, blank=True, max_length=255, db_index=True
    )
    searchable_synopsis = models.CharField(
        editable=False, default="", null=True, blank=True, max_length=255, db_index=True
    )
    ...
    value_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=VALUE_UNITS
    )
    ...
    length_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=DISTANCE_UNITS
    )
    length = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    ...
    weight_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=WEIGHT_UNITS
    )
    weight = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    temperature_rating_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=TEMPERATURE_UNITS
    )
    temperature_rating = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    temperature_rating_range_max = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    temperature_rating_range_min = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    wavelength_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=WAVELENGTH_UNITS
    )
    wavelength = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    frequency_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=FREQUENCY_UNITS
    )
    frequency = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    memory_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=MEMORY_UNITS
    )
    memory = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    interface = models.CharField(
        max_length=12, default=None, null=True, blank=True, choices=INTERFACE_TYPES
    )
    power_rating_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=POWER_UNITS
    )
    power_rating = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    supply_voltage_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=VOLTAGE_UNITS
    )
    supply_voltage = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    voltage_rating_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=VOLTAGE_UNITS
    )
    voltage_rating = models.DecimalField(
        max_digits=7, decimal_places=3, default=None, null=True, blank=True
    )
    current_rating_units = models.CharField(
        max_length=5, default=None, null=True, blank=True, choices=CURRENT_UNITS
    )
    ...
    def synopsis(self, return_displayable=True):
        return (
            self.displayable_synopsis
            if return_displayable
            else self.searchable_synopsis
        )
    ...
    def flat(self, top_level_quantity=100, sort=False):
        ...
        def sort_by_references(p):
            return (
                prep_for_sorting_nicely(p.references)
                if p.references
                else p.__str__().split()
            )
    ...
    def where_used(self):
        ...
        used_in_pr = PartRevision.objects.filter(
            assembly__in=used_in_assembly_ids
        ).order_by("-revision")
        return used_in_pr
    ...
    def __str__(self):
        return "{}, Rev {}".format(self.part.full_part_number(), self.revision)
...
class Subpart(models.Model):
    ...
    def __str__(self):
        return "{} {}".format(self.part_revision, self.count)


class Assembly(models.Model):
    subparts = models.ManyToManyField(
        Subpart, related_name="assemblies", through="AssemblySubparts"
    )
...
class ManufacturerPart(models.Model, AsDictModel):
    ...
    def seller_parts(self):
        return SellerPart.objects.filter(manufacturer_part=self).order_by(
    ...
    def __str__(self):
        return "%s" % (self.manufacturer_part_number)
...
class SellerPart(models.Model, AsDictModel):
    ...
    def order_quantity(self, extended_quantity):
        ...
        if (
            ...
            and extended_quantity > self.minimum_order_quantity
        ):
            order_qty = (
                ceil(extended_quantity / float(self.minimum_order_quantity))
                * self.minimum_order_quantity
            )
        return order_qty
