...
class PartBom(AsDictModel):
    ...
    def update_bom_for_part(self, bom_part):
        ...
        if bom_part.seller_part:
            ...
            self.out_of_pocket_cost = (
                self.out_of_pocket_cost + bom_part.out_of_pocket_cost()
            )
            self.nre_cost = (
                (self.nre_cost + bom_part.seller_part.nre_cost)
    ...
    def mouser_parts(self):
        ...
        for bom_id, item in self.parts.items():
            if (
                item.part.id not in mouser_items
                and item.part.number_class.mouser_enabled
            ):
...
class PartBomItem(AsDictModel):
    def __init__(
        ...
        self.quantity = (
            quantity  # quantity is the quantity per each direct parent assembly
        )
        ...
        self.order_cost = Money(
            0, self._currency
        )  # order_cost is updated similar to above order_quantity - Set when appending to PartBom
    ...
    def manufacturer_parts_for_export(self):
        return [
            mp.as_dict_for_export()
            for mp in self.part.manufacturer_parts(exclude_primary=True)
        ]

    def seller_parts_for_export(self):
        return [
            sp.as_dict_for_export()
            for sp in self.part.seller_parts(exclude_primary=True)
        ]
...
class PartIndentedBomItem(PartBomItem, AsDictModel):
    def __init__(
        self, indent_level, parent_id, subpart, parent_quantity, *args, **kwargs
    ):
