...
## Table of contents
   ...
   * [Clean up empty manufacturer parts](#clean-up-empty-manufacturer-parts)
...
1. Backup (see [Backup](#backup-and-restore-database-if-using-docker-compose-and-postgres)):

```
docker compose --env-file .env.prod exec -T db pg_dump -c -U bom_user bom_db | gzip > ./dump_bom_db_$(date +"%Y-%m-%d_%H_%M_%S").sql.gz
```

2. Dry-run (default, no writes). Review the counts:
